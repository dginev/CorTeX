...
// History immutability (owner directive 2026-06-15): there is intentionally NO agent endpoint that
// deletes/modifies the historical tables, so there is nothing to test on that surface — pruning the
// snapshot table is a human-admin-only operation, covered above.
