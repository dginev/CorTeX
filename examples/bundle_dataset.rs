...
fn main() {
  ...
  let corpus = match backend.sync(&corpus_placeholder) {
    Ok(c) => c,
    Err(e) => {println!("Failed to load corpus: {:?}", e); return; }
  };
  ...
  let service = match backend.sync(&service_placeholder) {
    Ok(s) => s,
    Err(e) => {println!("Failed to load service: {:?}", e); return; }
  };
  ...
  for status in vec![TaskStatus::NoProblem, TaskStatus::Warning, TaskStatus::Error].into_iter() {
    ...
    println!("Entries found for severity {:?}: {:?}", status.to_key(), entries.len());
              ...
              if is_well_formed { match entry_name_regex.captures(&entry) {
                Some(cap) => {
                  ...
                  match archive_writer_new.write_data(raw_entry_data) {
                    Ok(_) => {},
                    Err(e) => println!("Failed to write data to {:?} because {:?}", dataset_path.clone(), e)
                  };
                }
                None => {}
              }}
