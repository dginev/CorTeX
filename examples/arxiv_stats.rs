...
fn main() {
  ...
  let zero = 0 as u64;
  let ordered_counts = ORDERED_MONTHS.iter().map(|m| match arxiv_counts.get(
    &m.to_string(),
  ) {
    Some(counts) => counts,
    None => &zero,
  ...
  let ordered_sizes = ORDERED_MONTHS.iter().map(
    |m| match arxiv_monthly_sizes.get(
      &m.to_string(),
    ) {
      Some(&counts) => counts,
      None => zero,
  ...
  let freq_keys = arxiv_size_frequencies.clone().into_iter().map(
    |entry| entry.0,
  );
  let freq_values = arxiv_size_frequencies.clone().into_iter().map(
    |entry| entry.1,
  );
