//! During the August 2025, upgrade to Postgres 17 on our KWARC server that runs CorTeX builds for
//! ar5iv, there was a keyboard mistake that caused the DB to drop the session tables for the full
//! run.
//!
//! As a means of recovery, observe that all files remain on disk in case of DB mishaps, where each
//! converted task has a "cortex.log" file that persists all messages, including the final
//! conversion status.
//!
...
//!    preparing log_* messages in the "usual way" (reusing the code from the
//!    dispatcher/server/sink)
//! 3. The remaining TODO tasks are then either "Fatals" that came back without a log file, or have
//!    other issues. Ideally we actually re-convert those entries for certainty.
...
fn main() {
  ...
  eprintln!(
    "-- will scan and update {} tasks a batch of {} at a time",
    tasks.len(),
    batch_size
  );
  ...
  loop {
    ...
    for task in batch {
      let html_entry = ENTRY_ZIP_NAME_REGEX
        .replace(&task.entry, "tex_to_html.zip")
        .to_string();
    ...
    if success {
      eprintln!(
        "-- Successfully saved {} reports for {} tasks",
        reports_len, batch_len
      );
    }
  }
}
