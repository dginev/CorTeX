...
// generates the OpenAPI spec from them, writing it to docs/archive/api-spike/okapi-openapi.json.
//
...
fn main() {
  ...
  std::fs::create_dir_all("docs/archive/api-spike").expect("mkdir docs/archive/api-spike");
  std::fs::write("docs/archive/api-spike/okapi-openapi.json", &json).expect("write spec");
  println!(
    "rocket_okapi: wrote docs/archive/api-spike/okapi-openapi.json ({} bytes)",
    json.len()
  );
}
